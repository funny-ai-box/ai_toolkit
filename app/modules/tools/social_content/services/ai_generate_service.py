...
class AIGenerateService:
    ...
    async def generate_platform_contents_async(
        ...
        try:
            ...
            self.logger.error(f"为平台生成内容失败，任务ID：{task.id}, 平台ID：{task_platform.platform_id}: {str(ex)}", exc_info=True)
            raise
    ...
    async def generate_images_desc_async(self, task_images: List[GenerationTaskImage]) -> None:
        ...
        for image in task_images:
            try:
                ...
                self.logger.error(f"处理任务图片失败，图片ID：{image.id}: {str(ex)}", exc_info=True)
    ...
    async def search_related_contents_async(
        ...
        try:
            ...
            self.logger.error(f"搜索相关内容失败，任务ID：{task.id}: {str(ex)}", exc_info=True)
        return related_contents
